...
# 网络 web框架
# json http请求
# json就是协议 json是一种数据格式 协议 json.dumps() 序列化 json.loads() 反序列化 成dict list
class Company:
    name = "华为"
    address = "北京"
...
def print_Info_rpc(student):
    ...
    # 3.发送json字符串
    # 4.等待对方发送结果回来 json 去解析 反序列化 性能比较低 grpc高
    # 5.基于解析的结果进行业务逻辑
...
# http协议来说 有一个问题：一次性 一旦对方返回了结果 连接断开 http2.0 保持长连接 grpc
